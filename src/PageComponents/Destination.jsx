...
function Destination() {
  ...
  function loadDestination(x) {
    fetch(`${c.backendUrl}/destinations/${x}`).then(async (r) => {
      if (r.ok) {
        ...
        fetch(`${c.backendUrl}/destinations/inplace/${d[0].place.id}`).then(
          async (r) => {
            if (r.ok) {
              let od = await r.json();
              setOde(od);
        ...
        fetch(`${c.backendUrl}/facilities/inplace/${d[0].place.id}`).then(
          async (r) => {
            if (r.ok) {
              let od = await r.json();
              setFac(od);
  ...
  function getMyKind(d) {
    fetch(`${c.backendUrl}/likes/my-e/D-${d}`, {
  ...
  function actUpon(k) {
    fetch(`${c.backendUrl}/likes/D-${id}/${k}`, {
      method: 'POST',
      headers: { authtoken: localStorage.getItem('authtoken') },
    }).then(async (l) => {
      if (l.ok) {
  ...
  function getComments(d) {
    fetch(`${c.backendUrl}/comments/e/D-${d}`, {
      headers: { authtoken: localStorage.getItem('authtoken') },
    }).then(async (l) => {
      if (!l.ok) {
        let ll = await l.text();
  ...
  function makeComment(ent, mess) {
    ...
    fetch(`${c.backendUrl}/comments/D-${ent}`, {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="destination-page">
      ...
      <destnav style={{ display: 'flex', alignItems: 'center' }}>
        ...
        <account style={{}}>
          {loggedUser == null ? (
            <FontAwesomeIcon icon={faCircleNotch} className="rotating" />
          ) : loggedUser.error ? (
            <>
              <FontAwesomeIcon
                className="notlogged"
                ...
                onClick={() => {
                  gsap.fromTo(
                    ...
                    { opacity: 1, pointerEvents: 'all', y: -20 }
                  );
                }}
              />
              <SmallSignIn
            ...
            <loggeduser>
              <div
                ...
                style={{
                  ...
                  height: 40,
                  borderRadius: '50%',
                  background: `url('${c.backendUrl}${loggedUser.user.image}')`,
      ...
      {destination.state === 'waiting' ? (
        <FontAwesomeIcon size="5x" className="rotating" icon={faSpinner} />
        ...
        <>
          <img
            src={`${c.backendUrl}/files/imagebyid/${
              destination.dd.images.split(',')[im]
            }`}
            alt=""
            className="bgimg"
          />
            ...
            <div className="content">
              <div className="labelcontainer">
                <span className="location">
                  ...
                  &nbsp;{destination.dd.place.name},{' '}
                  {destination.dd.place.country}
                </span>
                <span className="name">{destination.dd.name}</span>
                <span className="description">
                  {destination.dd.description}
                </span>
                <div className="thumbnails">
                  {destination.dd.images.split(',').map((i, v) => (
                    <img
                      ...
                      src={`${c.backendUrl}/files/imagebyid/${i}`}
                      alt=""
                    />
                  ))}
                </div>
              ...
              <div className="contentcontainer">
                <div className="dcontent dconsel" id="near">
                  ...
                  >
                    <FontAwesomeIcon
                      icon={faSearch}
                      style={{ color: 'rgba(0,0,0,.5)', marginRight: 10 }}
                    />
                    <input
                      style={{
                        flex: '1',
                  ...
                  >
                    {ode
                      ...
                      .map((o) => (
                        <>
                          <a
                            href={`/destination/${o.id}`}
                            className="otherd"
                            onClick={() => loadDestination(o.id)}
                          >
                            <img
                              className="ctimg"
                              style={{
                      ...
                      <>
                        ...
                        >
                          <img
                            ...
                            src={`${c.backendUrl}/files/facility/imagebyid/${
                              o.images.split(',')[0]
                            }`}
                            alt=""
                          />
                        ...
                        <div className="ccm" id={`C-${co.id}`}>
                          ...
                          <div className="ccm-content">
                            <div className="ccm-user">
                              ...
                              <span className="ccm-date">
                                {moment(co.commentdate).fromNow(false)}
                              </span>
                            </div>
                            <div className="ccm-message">{co.message}</div>
                            <div className="ccm-actions">
                              <div
                                className="ccm-action"
                                onClick={() => {
                                  c.makeLike(`C-${co.id}`, 1).then(() => {
                                    c.getEntityLikes(`C-${co.id}`).then(
                                      async (l) => {
                                        if (!l.ok) {
                                          let e = await l.json();
                                          console.log(e);
                              ...
                              <div
                                ...
                                onClick={() => {
                                  setCmr([]);
                                  c.getEntityComments(`C-${co.id}`).then(
                                    async (cm) => {
                                      if (cm.ok) {
                                        let cmm = await cm.json();
                                        setCmr(cmm);
                                  ...
                                  if (
                                    ...
                                    document
                                      .querySelectorAll('decomreplies')
                                      .forEach((d) => {
                                        d.style.display = 'none';
                                      });
                                    document
                                      .querySelectorAll('decomreply')
                                      .forEach((d) => {
                                        d.style.display = 'none';
                                      });
                                    document.querySelector(
                              ...
                              >
                                ...
                                {co.replies == 0
                                  ...
                                  : co.replies == 1
                                  ? '1 Reply'
                                  : `${co.replies.toLocaleString()} Replies`}
                              </div>
                            ...
                            <decomreplies
                              id={`dcomrep-list-${co.id}`}
                              style={{ display: 'none' }}
                            >
                              {cmr.map((r) => (
                                <decomreplyitem className="decom-reply">
                                  <img
                                    className="decomreplyimg"
                                    src={`${c.backendUrl}${r.image}`}
                                    style={{
                                      width: 40,
                                      height: 40,
                                      borderRadius: '50%',
                                  ...
                                  <decomreplyitemcontent>
                                    <decomreplyitemu>
                                      <span>{r.name}</span>
                                      <span>
                                        {moment(r.commentdate).fromNow(false)}
                                      </span>
                                    </decomreplyitemu>
                                    <decomreplyitemc>
                                      {r.message}
                              ...
                              <button
                                onClick={() => {
                                  c.makeComment(
                                    `C-${co.id}`,
                                    document.querySelector(
                                      `#dcomreply-textarea-${co.id}`
                                    ).value
                                  ).then(async (cm) => {
                                    if (!cm.ok) {
                                      let cmm = await cm.text();
              ...
              <div className="optionscontainer">
                ...
                <div
                  ...
                  onClick={(e) => {
                    ...
                    if (comments.length > 0) {
                      document
                        .querySelector(`#ccmbottomdiv`)
                        .scrollIntoView({ behavior: 'smooth' });
                      document
                        .querySelector(`#ccmmbottomdiv`)
                        .scrollIntoView({ behavior: 'smooth' });
                    }
                  }}
                >
                ...
                <div
                  style={{
                    background:
                      myKind.length > 0 && myKind[0].kind == 1
                        ? 'rgba(14, 82, 34, 1)'
                        : 'transparent',
                  }}
                  className="option"
                ...
                >
                  ...
                  >
                    {dlikes.filter((l) => l.kind == 1).length.toLocaleString()}
                  </span>
                </div>
                <div
                  className="option"
                  style={{
                    background:
                      myKind.length > 0 && myKind[0].kind == 0
                        ? 'rgb(105, 41, 55)'
                        : 'transparent',
                  }}
                  onClick={() => {
                ...
                >
                  ...
                  >
                    {dlikes.filter((l) => l.kind == 0).length.toLocaleString()}
                  </span>
                </div>
